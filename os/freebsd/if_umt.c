/*- 
 * Copyright (c) 2016 Tom Jones <tj@enoti.me>
...
#ifdef	UMT_DEBUG
int run_debug = 0;
static SYSCTL_NODE(_hw_usb, OID_AUTO, umt, CTLFLAG_RW, 0, "USB umt");
SYSCTL_INT(_hw_usb_umt, OID_AUTO, debug, CTLFLAG_RWTUN, &umt_debug, 0,
    "umt debug level");
#endif
 ...
 */
#define	UMT_CMDQ_GET(c)	(atomic_fetchadd_32((c), 1) & UMT_CMDQ_MASQ)

static const STRUCT_USB_HOST_ID umt_devs[] = {
#define	UMT_DEV(v,p)	{ USB_VP(USB_VENDOR_##v, USB_PRODUCT_##v##_##p) }
#define	UMT_DEV_EJECT(v,p)	\
	{ USB_VPI(USB_VENDOR_##v, USB_PRODUCT_##v##_##p, UMT_EJECT) }
#define	UMT_EJECT	1
	UMT_DEV(MEDIATEK,		RT7610U),
#undef UMT_DEV_EJECT
#undef UMT_DEV
};

static device_probe_t	umt_match;
    ...
    struct usb_attach_arg *uaa)
{
	...
	if (id == NULL || id->bInterfaceClass != UICLASS_MASS)
		return;
	if (usbd_lookup_id_by_uaa(umt_devs, sizeof(umt_devs), uaa))
		return;
...
#endif
static int
umt_driver_loaded(struct module *mod, int what, void *arg)
{
	switch (what) {
	case MOD_LOAD:
	/*
...
umt_match(device_t self)
{
	...
	printf("%s:%d: scanning usb tree\n",__func__, __LINE__);
	return (usbd_lookup_id_by_uaa(umt_devs, sizeof(umt_devs), uaa));
}

static int
umt_attach(device_t self)
{
	printf("%s:%d: \n",__func__, __LINE__);
	struct umt_softc *sc = device_get_softc(self);
	struct usb_attach_arg *uaa = device_get_ivars(self);
...
static void
run_setup_tx_list(struct umt_softc *sc, struct run_endpoint_queue *pq)
{
...
static void
run_unsetup_tx_list(struct umt_softc *sc, struct run_endpoint_queue *pq)
{
	struct run_tx_data *data;
...
static int
run_load_microcode(struct umt_softc *sc)
{
...
static usb_error_t
run_do_request(struct umt_softc *sc,
...
static int
run_read(struct umt_softc *sc, uint16_t reg, uint32_t *val)
{
	uint32_t tmp;
	int error;
...
static int
run_read_region_1(struct umt_softc *sc, uint16_t reg, uint8_t *buf, int len)
{
	usb_device_request_t req;
...
static int
run_write_2(struct umt_softc *sc, uint16_t reg, uint16_t val)
{
	usb_device_request_t req;
...
static int
run_write(struct umt_softc *sc, uint16_t reg, uint32_t val)
{
	int error;
...
static int
run_write_region_1(struct umt_softc *sc, uint16_t reg, const uint8_t *buf,
    int len)
{
#if 1
	int i, error = 0;
...
static int
run_set_region_4(struct umt_softc *sc, uint16_t reg, uint32_t val, int len)
{
	int i, error = 0;
...
static int
run_efuse_read(struct umt_softc *sc, uint16_t addr, uint16_t *val, int count)
{
	uint32_t tmp;
	uint16_t reg;
	int error, ntries;
...
static int
run_efuse_read_2(struct umt_softc *sc, uint16_t addr, uint16_t *val)
{
	return (run_efuse_read(sc, addr, val, 2));
}

static int
run_eeprom_read_2(struct umt_softc *sc, uint16_t addr, uint16_t *val)
{
	usb_device_request_t req;
	uint16_t tmp;
	int error;
...
static device_method_t umt_methods[] = {
	...
	DEVMETHOD(device_attach,	umt_attach),
	DEVMETHOD(device_detach,	umt_detach),
	DEVMETHOD_END
};

static driver_t umt_driver = {
	.name = "umt",
	.methods = umt_methods,
	.size = sizeof(struct umt_softc)
};

static devclass_t umt_devclass;

DRIVER_MODULE(umt, uhub, umt_driver, umt_devclass, umt_driver_loaded, NULL);
MODULE_DEPEND(umt, wlan, 1, 1, 1);
MODULE_DEPEND(umt, usb, 1, 1, 1);
MODULE_DEPEND(umt, firmware, 1, 1, 1);
MODULE_VERSION(umt, 1);
USB_PNP_HOST_INFO(umt_devs);
